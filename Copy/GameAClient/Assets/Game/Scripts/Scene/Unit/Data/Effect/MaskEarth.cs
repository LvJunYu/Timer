...
//            LogHelper.Debug("OnTrigger {0} {1}", value, this);
        ...
        public override void UpdateLogic()
        {
            if (_trigger && _intersect)
            {
                ...
                if (mainPlayer != null)
                {
                    if (!ColliderGrid.Intersects(mainPlayer.ColliderGrid))
                    {
                        _intersect = false;
                        if (IsEdgeEmpty(EDirectionType.Up))
                        {
                            if (mainPlayer.ColliderGrid.YMin > ColliderGrid.YMax)
                            {
                                OnTrigger(false);
                            }
                        }
                        ...
                        {
                            if (mainPlayer.ColliderGrid.YMax < ColliderGrid.YMin)
                            {
                                OnTrigger(false);
                            }
                        }
                        ...
                        {
                            if (mainPlayer.ColliderGrid.XMax < ColliderGrid.XMin)
                            {
                                OnTrigger(false);
                            }
                        }
                        ...
                        {
                            if (mainPlayer.ColliderGrid.XMin > ColliderGrid.XMax)
                            {
                                OnTrigger(false);
                            }
                        }
