...
namespace GameA.Game
{
    ...
    [Serializable]
    public class UnitBase : UnitEntity, IEquatable<UnitBase>
    {
...
//        }
        internal bool Init(Table_Unit tableUnit, UnitDesc unitDesc, SceneNode dynamicCollider)
        {
            ...
            UpdateExtraData();
            OnInit();
            ColliderGridInner = _useCorner ? ColliderGrid.GetGridInner() : ColliderGrid;
            return true;
        }
        ...
        internal override void Reset()
        {
            ...
            _curPos = new IntVec2(_guid.x, _guid.y);
            ColliderPos = GetColliderPos(_curPos);
            ColliderGrid = _tableUnit.GetColliderGrid(ref _unitDesc);
            if (_dynamicCollider != null && !LastColliderGrid.Equals(ColliderGrid))
            {
...
//            }
            ...
            ColliderPos = GetColliderPos(_curPos);
            LastColliderGrid = ColliderGrid = _tableUnit.GetColliderGrid(ref _unitDesc);
            ColliderGridInner = _useCorner ? ColliderGrid.GetGridInner() : ColliderGrid;
        ...
        public virtual UnitExtraDynamic UpdateExtraData(UnitExtraDynamic unitExtraDynamic = null)
        {
            ...
            {
                _componentList[i].UpdateExtraData(unitExtraDynamic);
            }
        ...
        public static bool operator ==(UnitBase a, UnitBase b)
        {
            ...
            if (((object) a == null) && b.IsDisposed)
            {
                return true;
            }

            if (((object) b == null) && a.IsDisposed)
            {
                return true;
            }
        ...
        public void UpdateTransPos(IntVec2 pos)
        {
            if (_view != null)
            {
                _trans.position = GetTransPos(pos);
            }
        }
        ...
        public void UpdateTransPos()
        {
            if (_view != null)
            {
                _trans.position = GetTransPos(_curPos);
            }
        }

        public Vector3 GetTransPos(IntVec2 pos)
        {
            ...
            {
                return GM2DTools.TileToWorld(pos) + _tableUnit.ModelOffset + new Vector3(0, 0.5f, z);
            }

            if (UnitDefine.IsDownY(_tableUnit))
            {
                return GM2DTools.TileToWorld(pos) + _tableUnit.ModelOffset + new Vector3(0, -0.1f, z);
            }

            return GM2DTools.TileToWorld(pos) + _tableUnit.ModelOffset + Vector3.forward * z;
        }
        ...
        {
            var z = GetZ(ColliderPos);
            if (IsInWater)
            {
                var tile = new IntVec2(ColliderPos.x / ConstDefineGM2D.ServerTileScale,
                    ColliderPos.y / ConstDefineGM2D.ServerTileScale);
            ...
            {
                var tile = ColliderPos / ConstDefineGM2D.ServerTileScale;
        ...
        public virtual bool SetInputByMoveDir(EMoveDirection eMoveDirection)
        {
            return false;
        }
        ...
        {
            var min = new IntVec2(ColliderGrid.XMin + deltaX, ColliderGrid.YMin);
            return new Grid2D(min.x, min.y, min.x + ColliderGrid.XMax - ColliderGrid.XMin,
                min.y + ColliderGrid.YMax - ColliderGrid.YMin);
        }
        ...
        {
            var min = new IntVec2(ColliderGrid.XMin, ColliderGrid.YMin + deltaY);
            return new Grid2D(min.x, min.y, min.x + ColliderGrid.XMax - ColliderGrid.XMin,
                min.y + ColliderGrid.YMax - ColliderGrid.YMin);
        }
        ...
        protected bool CheckRightClimbFloor(int deltaPosY = 0)
        {
            ...
            var min = new IntVec2(ColliderGrid.XMax + 1, CenterPos.y + deltaPosY);
        ...
        {
            if (!CanClimb) return false;
            var min = new IntVec2(ColliderGrid.XMin - 1, CenterPos.y + deltaPosY);
            var grid = new Grid2D(min.x, min.y, min.x, min.y);
            using (var units = ColliderScene2D.GridCastAllReturnUnits(grid,
                JoyPhysics2D.GetColliderLayerMask(_dynamicCollider.Layer), float.MinValue, float.MaxValue,
                _dynamicCollider))
        ...
        {
            if (!CanClimb) return false;
            var min = new IntVec2(CenterPos.x + deltaPosX, ColliderGrid.YMax + 1);
            var grid = new Grid2D(min.x, min.y, min.x, min.y);
            using (var units = ColliderScene2D.GridCastAllReturnUnits(grid,
                JoyPhysics2D.GetColliderLayerMask(_dynamicCollider.Layer), float.MinValue, float.MaxValue,
                _dynamicCollider))
        ...
        {
            return ColliderGrid.YMax + 1 == unit.ColliderGrid.YMin && ColliderGrid.XMax >= unit.ColliderGrid.XMin &&
                   ColliderGrid.XMin <= unit.ColliderGrid.XMax;
        }

        protected bool CheckLeftFloor(UnitBase unit)
        {
            return ColliderGrid.XMin - 1 == unit.ColliderGrid.XMax && ColliderGrid.YMax >= unit.ColliderGrid.YMin &&
                   ColliderGrid.YMin <= unit.ColliderGrid.YMax;
        }

        protected bool CheckRightFloor(UnitBase unit)
        {
            return ColliderGrid.XMax + 1 == unit.ColliderGrid.XMin && ColliderGrid.YMax >= unit.ColliderGrid.YMin &&
                   ColliderGrid.YMin <= unit.ColliderGrid.YMax;
        }
        ...
        public IntVec2 GetPos(IntVec2 colliderPos)
        {
...
//            RpgTaskManger.Instance.OnControlFinish(_guid);
        ...
        public virtual void Hit(UnitBase unit, EDirectionType eDirectionType)
        {
        }
        ...
        public bool CheckUpValid(ref int speedY, ref UnitBase unit, bool checkOnly = false)
        {
            ...
            GM2DTools.GetBorderPoint(ColliderGrid, EDirectionType.Up, ref pointACheck, ref pointBCheck);
                ...
                float.MinValue, float.MaxValue, _dynamicCollider))
            {
                for (int i = 0; i < units.Count; i++)
                {
                    ...
                    if (unitHit.IsAlive)
                    {
                        ...
                        {
                            if (unitHit.ColliderGrid.YMin > ColliderGrid.YMax + 1)
                            {
                                if (!checkOnly)
                                {
                                    speedY = 0;
                            ...
                            else if (unitHit.ColliderGrid.YMin == ColliderGrid.YMax + 1)
                            {
