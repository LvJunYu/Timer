...
namespace GameA
{
    public class UPCtrlWorldOfficalSelect : UPCtrlWorldProjectBase
    {
        private WorldOfficialRecommendProjectList _data;

        public override void RequestData(bool append = false)
        {
            base.RequestData(append);
            _data = AppData.Instance.WorldData.WorldOfficialProjectList;
            int startInx = 0;
            if (append)
            {
