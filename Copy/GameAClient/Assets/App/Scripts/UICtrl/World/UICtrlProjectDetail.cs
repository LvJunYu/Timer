...
namespace GameA
{
    ...
    public class UICtrlProjectDetail : UICtrlAnimationBase<UIViewProjectDetail>, ICheckOverlay
    {
        ...
        protected override void OnViewCreated()
        {
            ...
            _cachedView.RecommendWaitingBtn.onClick.AddListener(OnRecommendWaitingBtn);
            _cachedView.RecommendPrepareBtn.onClick.AddListener(OnRecommendPrepareBtn);
            _cachedView.RemoveWaitingBtn.onClick.AddListener(OnRemoveRrepareBtn);
            _cachedView.UpdataProjectBtn.onClick.AddListener(OnUpdataProjectBtn);
            _cachedView.RemoveProjectBtn.onClick.AddListener(OnRemoveProjectBtn);
        }
...
//            }
        ...
        private void SetRPGProject(bool isrpg)
        {
        ...
        private void RefreshGMModle()
        {
            _cachedView.GMPanel.SetActiveEx(OfficalData.Instance.IsGmModle && !_project.IsMaster);
            if (!OfficalData.Instance.IsGmModle || _project.IsMaster)
            {
                return;
            }

            RefreshGMProject(EWorldOfficialRecommendPrepareProjectStatus.EWORPPS_None);

            GmProjectOfficialRecommendStatus recommendstatus = new GmProjectOfficialRecommendStatus();
            recommendstatus.Request(_project.ProjectId, () => { RefreshGMRecommendStatus(recommendstatus); },
                code => { SocialGUIManager.ShowPopupDialog("无法取关卡推荐状态！"); });
        }

        private void OnRecommendWaitingBtn()
        {
            List<long> projectList = new List<long>();
            projectList.Add(_project.MainId);
            RemoteCommands.GmUpdateProjectOfficialRecommendStatus(projectList,
        ...
        private void OnRecommendPrepareBtn()
        {
            List<long> projectList = new List<long>();
            projectList.Add(_project.MainId);
            RemoteCommands.GmUpdateProjectOfficialRecommendStatus(projectList,
                ...
                {
                    RefreshStatus();
                    SocialGUIManager.ShowPopupDialog("推荐成功！");
                },
                code => { SocialGUIManager.ShowPopupDialog("推荐失败"); });
        }

        private void OnRemoveRrepareBtn()
        {
            List<long> projectList = new List<long>();
            projectList.Add(_project.MainId);
            RemoteCommands.GmUpdateProjectOfficialRecommendStatus(projectList,
                ...
                {
                    RefreshStatus();
                    SocialGUIManager.ShowPopupDialog("移除推荐成功！");
                },
                code => { SocialGUIManager.ShowPopupDialog("移除推荐失败"); });
        }

        private void OnUpdataProjectBtn()
        {
            List<long> _list = new List<long>();
            _list.Add(_project.ProjectId);
            RemoteCommands.GmUpgradeOfficialRecommendProject(_list,
                ret => { SocialGUIManager.ShowPopupDialog("更新成功！"); },
                code => { SocialGUIManager.ShowPopupDialog("更新失败"); });
        }

        private void OnRemoveProjectBtn()
        {
            List<long> _list = new List<long>();
            _list.Add(_project.ProjectId);
            RemoteCommands.GmUpgradeOfficialRecommendProject(_list,
                ret => { SocialGUIManager.ShowPopupDialog("待选成功！"); },
                code => { SocialGUIManager.ShowPopupDialog("待选失败"); });
        }

        public void RefreshGMProject(EWorldOfficialRecommendPrepareProjectStatus status)
        {
            switch (status)
            {
                case EWorldOfficialRecommendPrepareProjectStatus.EWORPPS_HasUpdate:
                    _cachedView.UpdataProjectBtn.SetActiveEx(true);
                    break;
                case EWorldOfficialRecommendPrepareProjectStatus.EWORPPS_HasDeleted:
                    _cachedView.RemoveProjectBtn.SetActiveEx(true);
                    break;
                case EWorldOfficialRecommendPrepareProjectStatus.EWORPPS_Normal:
                case EWorldOfficialRecommendPrepareProjectStatus.EWORPPS_None:
                    _cachedView.RemoveProjectBtn.SetActiveEx(false);
                    _cachedView.UpdataProjectBtn.SetActiveEx(false);
                    break;
            }
        }

        private void RefreshGMRecommendStatus(GmProjectOfficialRecommendStatus status)
        {
            _cachedView.RecommendWaitingBtn.SetActiveEx(false);
            _cachedView.RemoveWaitingBtn.SetActiveEx(false);
            _cachedView.RecommendPrepareBtn.SetActiveEx(false);
            switch (status.Status)
            {
                case EOfficialRecommendProjectStatus.EORPS_Candidate:
                    _cachedView.RemoveWaitingBtn.SetActiveEx(true);
                    _cachedView.RecommendPrepareBtn.SetActiveEx(true);
                    break;
                case EOfficialRecommendProjectStatus.EORPS_None:
                    _cachedView.RecommendWaitingBtn.SetActiveEx(true);
                    break;
                case EOfficialRecommendProjectStatus.EORPS_Prepare:
                    break;
//                case EOfficialRecommendProjectStatus.EORPS_Recommend:
//                    break;
            }
        }
