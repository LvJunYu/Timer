...
namespace GameA
{
    public class UMCtrlProjectGMWrokShop : UMCtrlBase<UMViewProjectSelfCommend>, IDataItemRenderer
    {
        private CardDataRendererWrapper<GmPrepareProject> _wrapper;
        private int _index;
        ...
        protected override void OnViewCreated()
        {
            ...
            _cachedView.AddProjectBtn.onClick.AddListener(() =>
            {
                SocialGUIManager.Instance.GetUI<UICtrlOfficalWorkShop>().AddOfficialAdventure();
            });
        ...
        public void Set(object obj)
        {
            ...
            _wrapper = obj as CardDataRendererWrapper<GmPrepareProject>;
            if (_wrapper != null)
            {
                _wrapper.OnDataChanged += RefreshView;
            }
        ...
        {
            _cachedView.ProjectDragHelper.SetEnableEx(enable);
        }
        ...
        {
            bool emptyProject = _wrapper.Content.ProjectData.ProjectId == 0;
            if (!emptyProject)
            {
        ...
        private void ResponseEditBtn()
        {
            if (_wrapper != null)
            {
                ...
                {
                    if (_wrapper.Content.ProjectData.ProjectId != 0)
                    {
        ...
        {
            SocialGUIManager.Instance.GetUI<UICtrlOfficalWorkShop>().OnSelectBtn(Index);
            _cachedView.UnSelectBtn.SetActiveEx(true);
        }

        private void OnUnSelectBtn()
        {
            SocialGUIManager.Instance.GetUI<UICtrlOfficalWorkShop>().OnUnSelectBtn(Index);
            _cachedView.UnSelectBtn.SetActiveEx(false);
        }
...
//            }
            ...
            if (_newindex != -1 && _benginIndex != _newindex)
            {
                ...
                SocialGUIManager.Instance.GetUI<UICtrlOfficalWorkShop>().OnDragEnd(_benginIndex, _newindex);
            }
            else
            {
                _gridDataScroller.EndTween();
                ...
                SocialGUIManager.Instance.GetUI<UICtrlOfficalWorkShop>().OnDragEnd(0, 0);
            }
