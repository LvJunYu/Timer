...
namespace GameA.Game
{
    public partial class UMViewGamePlayItem: UMViewBase
    {
        ...
        {
	        if (_condition == EWinCondition.TimeLimit)
	        {
		        return;
	        }
            bool value = _refData.SettingValue[(int) _condition];
        ...
        protected int GetBuildCount()
        {
            switch (_condition)
            {
                ...
                case EWinCondition.CollectTreasure:
                    {
                        return EditMode.Instance.MapStatistics.GemCount;
                    }
                case EWinCondition.KillMonster:
                    {
                        return EditMode.Instance.MapStatistics.MonsterCount;
                    }
                case EWinCondition.RescueHero:
                    {
                        return EditMode.Instance.MapStatistics.HeroCageCount;
                    }
                default:
                    {
                        return 0;
