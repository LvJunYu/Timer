...
namespace YIMEngine
{
	...
	{
		void OnSendMessageStatus(ulong iRequestID,  YIMEngine.ErrorCode errorcode, bool isForbidRoom, int reasonType, ulong forbidEndTime);
		//接收到用户发来的消息
		...
		//语音发送回调
		void OnSendAudioMessageStatus(ulong iRequestID,  YIMEngine.ErrorCode errorcode,string strText,string strAudioPath,int iDuration, bool isForbidRoom, int reasonType, ulong forbidEndTime);
