/********世豪*************/
using UnityEngine;
using System.Collections;
using SoyEngine;
using UnityEngine.UI;

namespace GameA
{
	public class USViewShopping : USViewBase
	{
		/// <summary>
		/// 上衣按钮/场景
		/// </summary>
		public Button SYbutton;
		public GameObject  SYbuttonDock;

		/// <summary>
		/// 裤子按钮/场景
		/// </summary>
		public Button KZbutton;
		public GameObject  KZbuttonDock;

		/// <summary>
		/// 鞋子按钮/场景
		/// </summary>
		public Button XZbutton;
		public GameObject  XZbuttonDock;

		/// <summary>
		/// 装饰按钮/场景
		/// </summary>
		public Button ZSbutton;
		public GameObject  ZSbuttonDock;

		/// <summary>
		/// 帽子按钮/场景
		/// </summary>
		public Button MZbutton;
		public GameObject  MZbuttonDock;

		/// <summary>
		/// 购买按钮
		/// </summary>
//		public Button BuyBtn;
//		public GameObject BuyBtnVive;
//		public Button BuyBtnViveBtn;
//		public GameObject BuyBtnViveBtnVive;
		/// <summary>
		/// 使用按钮
		/// </summary>
//		public Button USBtn;
		 
		/// <summary>
		/// 穿戴按钮
		/// </summary>
//		public Button CDBtn;
//		public GameObject CDBtnVive;
		/// <summary>
		/// Texture点击按钮试穿
		/// </summary>
//		public Button DressBtn;





		/*四个界面显示文字*/
		/// <summary>
		/// 右上角倒计时
		/// </summary>
//		public Text DjsText;
		/// <summary>
		/// 时装简介名字
		/// </summary>
//		public Text JiText;
		/// <summary>
		/// 价格
		/// </summary>
//		public Text price;
		/// <summary>
		/// 限时打折券
		/// </summary>
//		public Text Limit;





		public UITagGroup TagGroup;
//		public UICommentInput CommentInput;

		/// <summary>
		/// 卡片的父物体的位置（1）
		/// 剩下的还有4个界面
		/// </summary>
		 public RectTransform SYDock;








	}
}