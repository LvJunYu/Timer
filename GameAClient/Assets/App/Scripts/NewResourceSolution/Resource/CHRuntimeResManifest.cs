    ...
    public class CHRuntimeResManifest : CHResManifest
    {
		...
		public void MapBundles ()
		{
			...
			for (int i = 0; i < _bundles.Count; i++)
			{
				...
				{
					string registAssetName = _bundles[i].IsLocaleRes ?
						StringUtil.Format(StringFormat.TwoLevelPath, _bundles[i].LocaleName, _bundles[i].AssetNames[j]) :
						_bundles[i].AssetNames[j];
					if (!_assetName2BundleName.ContainsKey(registAssetName))
					{
//						LogHelper.Info("Regist assetName {0} with {1}", registAssetName, _bundles[i].AssetBundleName);
						_assetName2BundleName.Add(registAssetName, _bundles[i].AssetBundleName);
					}
					else
					{
		...
		private CHResBundle CacheBundleAndDependencies (CHResBundle bundle, int scenary)
		{
			...
			// 如果资源不在本地，也不是非压缩且在streamingAsset中
