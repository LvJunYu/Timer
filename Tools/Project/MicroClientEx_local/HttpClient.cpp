...
int CHttpClient::ExecuteRequest(LPCTSTR strMethod, LPCTSTR strUrl, LPCTSTR strPostData, LPCTSTR &strResponse)
{
    CString strServer;
    CString strObject;
    DWORD dwServiceType;
	...
	//该函数解析URL字符串并返回服务的类型及组件 包含在afxinet.h头文件中
	//strUrl字符串指针，指向要解析的URL dwservicetype 指示互联网服务的类型
	//strserver接受待解析的URL服务器名，服务类型后的第一个部分
...
int CHttpClient::HttpGet(LPCTSTR strUrl, LPCTSTR strPostData, LPCTSTR &strResponse)
{
    return ExecuteRequest("GET", strUrl, strPostData, strResponse);
}

int CHttpClient::HttpPost(LPCTSTR strUrl, LPCTSTR strPostData, LPCTSTR &strResponse)
{
