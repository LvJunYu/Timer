...
namespace Chronos
{
    ...
    public abstract class RigidbodyTimeline<TComponent, TSnapshot> : RecorderTimeline<TComponent, TSnapshot>,
        IRigidbodyTimeline where TComponent : Component
    {
        ...
        public float drag
        {
            ...
            set
            {
                if (AssertForwardProperty("drag", Severity.Warn)) realDrag = value * timeline.timeScale;
            }
        }
        ...
        public float angularDrag
        {
            ...
            set
            {
                if (AssertForwardProperty("angularDrag", Severity.Warn)) realAngularDrag = value * timeline.timeScale;
            }
        }
        ...
        protected bool AssertForwardProperty(string property, Severity severity)
        {
            if (timeline.timeScale <= 0)
            {
                ...
                {
                    throw new ChronosException("Cannot change the " + property +
                                               " of the rigidbody while time is paused or rewinding.");
                }
                else if (severity == Severity.Warn)
                {
                    Debug.LogWarning("Trying to change the " + property +
                                     " of the rigidbody while time is paused or rewinding, ignoring.");
                }
            }
            ...
            if (timeline.timeScale <= 0)
            {
                if (severity == Severity.Error)
                {
                    throw new ChronosException(
                        "Cannot apply a force to the rigidbody while time is paused or rewinding.");
                }
                else if (severity == Severity.Warn)
                {
                    Debug.LogWarning(
                        "Trying to apply a force to the rigidbody while time is paused or rewinding, ignoring.");
                }
            }
